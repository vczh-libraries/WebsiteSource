...
The home page loads `/index.json` from the site root. Its script uses a fixed JSON structure rather than the shared XML article parser. Keep visible content and image URLs in that data file; the HTML retains fixed controls and tab headers. GUI and TUI appearances share the same content and DOM, with presentation controlled by CSS. The logo retains its original 320 by 128 pixels; the header wraps around it on narrow screens. Tab contents are constructed as needed. A screenshot is requested only when its tab is selected and the gallery is on screen; its image node is retained after the first load.

`index.json` contains `text` (HTML element IDs mapped to text), navigation and action links, three `examples` (`hello`, `binding`, and `viewmodel`), `platforms`, quick links, coding-agent setup and language-integration guide links, and feature group boxes. Each platform has a stable `id`, source link, GUI icon, and `modes`; each mode has `themes` with IDs matching the fixed HTML tab headers, image URLs, alt text, and dimensions. GUI icons switch to text glyphs through CSS in TUI mode; their source and license are recorded in `assets/homeres/icons/README.md`. GitHub links open in a new browsing context with `rel="noopener"`.

Each example has `sources` (an ordered array of `{ id, label, code }`), a `resultLabel`, a `docs` link, and a `snapshot`. Source files are selected with native radio buttons in a vertical list; the selected label has a right-pointing triangle. Keyboard navigation and focus work in both GUI and TUI appearances, and switching example tabs preserves the selected source. The first source is selected initially. Hello world and Data binding each show `Instance.xml`; View model also offers `ViewModel.xml` with its `IHelloViewModel` interface, followed by class-only C++, Workflow, and TypeScript implementations. The view model's `Name` setter updates its observable `Welcome` property, with the window binding its text box and label through a view model property.

Each example pairs its XML Instance definition with a real Direct2D screenshot in `assets/homeres/samples/`; the View model screenshot uses the C++ implementation. These are static results, not browser simulations. Screenshots are horizontally centered at the top of their preview area, inside its padding. Keep the displayed source synchronized with the captured resource. Sample screenshots and GUI icons load only when visible and retain their image nodes after loading.

The TypeScript sample is a class excerpt with assumed imports and generated contract declarations. It is copied as a static asset and excluded from the website package's type-aware linting.
